...
for stock in symbols:
    ...
    market_cap = float(df_spot.loc[df_spot["代码"] == symbol_code, "总市值"].values[0])
    ...
    print(f"DCF 模型估算{stock}企业价值 (EV): {ev:,.2f} 元")
    print(f"{stock}当前总市值: {market_cap:,.2f} 元")
    print(f"{stock}估值/市值 比率: {ev/market_cap:.2f}")
